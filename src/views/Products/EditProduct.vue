<template>
      ...
      <div class="row">
        <div class="col-12 col-lg-6 pe-3 pe-lg-5">
          <div class="dash-step-content">
            <EditPages :steps="steps" :info="info" :fields="fields" :active="active"></EditPages>
  ...
  <div class="dash-dailog publish-product-dailog" :class="{ active: isActive }">
    <div class="dash-dailog-box">
      ...
      <div class="title">Thanks For Posting The Item</div>
      <p class="description">You Can Review Item Updates From Your Profile</p>
...
import StepsBanner from "../../components/Products/Sales/StepsBanner.vue";
import EditPages from "./EditPages.vue";

export default {
  
  components: { StepsBanner, EditPages },
  data() {
    return {
      isActive: false,
  ...
  methods: {
	...
	computed: {
		...mapGetters('product', ['info', 'selectedPackages']),

    info() {
      return this.$store.getters["product/info"]
    },
    fields() {
      return this.$store.getters["category/getFields"](
        this.info.category_id,
        this.active - 1
      );
    },
    steps() {
      return this.$store.getters["category/getPages"](this.info.category_id);
    },
    active() {
...
.dash-sell-form .form-feilds .form-step-title span.price {
  color: #3eadb7;
  font-weight: 400;
  margin-left: 40px;
}

.dash-sell-form .form-feilds .form-step-title.with-icon {
  display: flex;
  align-items: center;
...
.dash-sell-form .form-feilds .form-step-title.with-icon .icon {
  cursor: pointer;
}

.dash-sell-form .form-feilds .form-step-title.with-icon .description-subject {
  padding: 30px;
  position: absolute;
  top: 100%;
...
.dash-sell-form
  .form-feilds
  .form-step-title.with-icon.active
  .description-subject {
  display: block;
}
...
.form-feilds button.verify-google-btn {
  width: 100%;
  color: #9d9d9d;
...
.form-feilds .upload-images div.image img {
  display: block;
  margin: auto;
  max-width: 100%;
  border-radius: 5px;
...
.dash-step-contents .dash-sell-tips .tips-content {
  margin-left: 15px;
  margin-bottom: 60px;
  color: #9d9d9d;
  line-height: 40px;
...
.dash-sell-form button.next-btn,
.dash-sell-form button.publish-btn {
  width: 100%;
  background: #3eadb7;
  color: #fff;
}

.dash-sell-form .radio-boxes {
  display: flex;
  flex-wrap: wrap;
  align-items: stretch;
...
.dash-sell-form .radio-boxes label.radio-box {
  display: flex;
...
.dash-sell-form .radio-boxes label.radio-box .price {
  font-size: 60px;
  font-weight: bold;
  color: #040506;
  width: 100%;
...
.dash-dailog.publish-product-dailog.active {
  display: flex;
}
