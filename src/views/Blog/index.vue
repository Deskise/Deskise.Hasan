...
export default {
  ...
  computed: {
    ...
    posts() {
      let data = this.getPostsByCategoryId(this.category)
      let SearchPosts=[]
      for (let i = 0; i < data.length; i++){
        if (data[i].title.toLowerCase().includes(this.textSearch.toLowerCase())) {
          SearchPosts.push(data[i]);
        }
      }
      return SearchPosts;
    },
