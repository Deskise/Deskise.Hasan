...
export default {
  ...
  computed: {
    ...mapState("data", ["faq"]),
    getFaqs() {
      let data = JSON.parse(JSON.stringify(this.faq.data))
      let SearchFAQ=[]
      for (let i = 0; i < data.length; i++){
        if (data[i].question.toLowerCase().includes(this.textSearch.toLowerCase())) {
          SearchFAQ.push(data[i]);
        }
      }
      return SearchFAQ
    }
