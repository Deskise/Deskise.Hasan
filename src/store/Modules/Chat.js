...
export const actions = {
  ...
  async agreementResponse(context, { agreement, chatId, type }) {
    console.log(context);
    await Chat.chat().send().agreementResponse(agreement, chatId, type);
  },
...
export const getters = {
  ...
  blocked: (state, getters, rootState) => {
    return state.chats.filter((chat) => {
      return chat.blocker.some(
        (blocker) => blocker.id === rootState.user.data.id
      );
    });
  },
